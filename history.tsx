...
export default function HistoryScreen() {
  ...
  useEffect(() => {
    ...
    if (!selectedStillVisible) {
      setSelectedGameId(displayedGames[0]?.id);
    }
  }, [displayedGames, selectedGameId]);

  const selectedGame = useMemo(
    () => displayedGames.find((game) => game.id === selectedGameId),
    [displayedGames, selectedGameId]
  );
  ...
  const handleExportBackup = async () => {
    try {
      ...
      Alert.alert(
        'Export complete',
        `Backup export opened for ${normalizedFileName}. Choose Google Drive in the share sheet to pick the Drive location.`
      );
    } catch (error: any) {
      console.error(error);
      Alert.alert(
  ...
  const handleImportBackup = async () => {
    try {
      const imported = await importAndMergeBackup(useStore);
  ...
  const handleReplayGraph = () => {
    if (!selectedGame?.id) {
      Alert.alert(
        'Replay unavailable',
        'Select a saved game first to open the replay graph.'
      );
      return;
    }
          ...
          {backupExpanded ? (
            <>
              <Text style={styles.sectionSubtitle}>
                Export locally, import a backup, connect Drive, or upload a fresh
        ...
        <View style={styles.section}>
          ...
          <View style={styles.selectionPanel}>
            <View style={styles.selectionTextWrap}>
              <Text style={styles.selectionLabel}>Selected Game</Text>
              <Text style={styles.selectionTitle}>
                {selectedGame?.id
                  ? `Game ${displayedGames.findIndex((game) => game.id === selectedGame.id) >= 0
                      ? displayedGames.length - displayedGames.findIndex((game) => game.id === selectedGame.id)
                      : ''}`
                  : 'None selected'}
              </Text>
              <Text style={styles.selectionSubtitle}>
                {selectedGame
                  ? getPreviewText(selectedGame, players)
                  : 'Tap a saved game below, then open Replay Graph.'}
              </Text>
            </View>
            ...
            <Pressable
              style={[
                styles.replayButton,
                !selectedGame?.id && styles.replayButtonDisabled,
              ]}
              onPress={handleReplayGraph}
              disabled={!selectedGame?.id}
            >
              <Text style={styles.replayButtonText}>Replay Graph</Text>
            </Pressable>
          </View>
