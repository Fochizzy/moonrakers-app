...
import { buildDerivedPlayerStats, type DerivedPlayerStats } from '@/utils/derivedMetricsEngine';
...
function getCorrelationMeaning(label: string, value: number) {
  ...
  if (value > 0) {
    return `${getCorrelationStrength(value)} positive relationship. Higher ${lower} tends to line up with more winning.`;
  }

  return `${getCorrelationStrength(value)} negative relationship. Higher ${lower} tends to line up with less winning.`;
}
  ...
  return (
    <View style={[styles.correlationCard, { borderColor: `${accent}55` }]}>
      <View style={styles.correlationHeader}>
        ...
        <View style={[styles.correlationBadge, { borderColor: `${accent}66`, backgroundColor: `${accent}16` }]}>
          <Text style={[styles.correlationBadgeText, { color: accent }]}>
            {formatCorrelationValue(row.value)} � {getCorrelationStrength(row.value)} {tone}
          </Text>
        </View>
      </View>
...
function getGameRows(game: any) {
  ...
  return Object.entries(totals).map(([playerId, stats]: any) => ({
    playerId,
    prestige:
      safeMetric(stats?.totalPrestige ?? stats?.prestige ?? stats?.directPrestige) +
...
export default function StatsScreen() {
  ...
  const [activeCorrelationTab, setActiveCorrelationTab] = useState<CorrelationTab>('global');
  const [selectedCorrelationPlayerId, setSelectedCorrelationPlayerId] = useState<string | null>(null);
  const [selectedGameId, setSelectedGameId] = useState<string | null>(null);
const [selectedPlayerId, setSelectedPlayerId] = useState<string | null>(null);

  const players = useStore((s: any) => (Array.isArray(s.players) ? s.players : [])) as Player[];
  const games = useStore((s: any) => (Array.isArray(s.games) ? s.games : [])) as Game[];
  ...
  const baseLeaderboard = useMemo(() => buildLeaderboard(players, games), [players, games]);
  ...
  const resolvedCorrelationPlayerId = selectedCorrelationPlayerId ?? leaderboard[0]?.id ?? null;
  const resolvedPlayerId = selectedPlayerId ?? leaderboard[0]?.id ?? null;const individualCorrelations = useMemo(
    () => buildIndividualCorrelations(leaderboard, resolvedCorrelationPlayerId ?? undefined),
    [leaderboard, resolvedCorrelationPlayerId]
  );

  const selectedCorrelationPlayer = useMemo(
    () => leaderboard.find((p) => p.id === resolvedCorrelationPlayerId) ?? null,
    [leaderboard, resolvedCorrelationPlayerId]
  );

  
  const selectedPlayer = useMemo(
    () => leaderboard.find((p) => p.id === resolvedPlayerId) ?? null,
    [leaderboard, resolvedPlayerId]
  );

  const selectableGames = useMemo(
    () => buildSelectableGames(games),
    [games]
  );
...
const gameCorrelations = useMemo(
  ...
  const renderOverviewTab = () => (
    <View style={styles.card}>
      ...
      <Text style={styles.subtitle}>
        Fast league totals, top signals, and seat-order context in the same fleet-command theme as home.
      </Text>

      <Pressable style={styles.compareButtonTop} onPress={() => router.push('/charts/compare')}>
      ...
      <View style={styles.signalSection}>
        ...
        <View style={styles.signalList}>
          {topSignals.map((signal: any, index: number) => {
            ...
            return (
              <View key={signal.key} style={[styles.signalCard, { borderColor: `${accent}55` }]}>
                ...
                <View style={styles.signalBody}>
                  ...
                  <Text style={[styles.signalValue, { color: accent }]}>
                    {formatCorrelationValue(signal.value)} � {getCorrelationStrength(signal.value)}
                  </Text>
                </View>
              </View>
            );
    ...
    if (!leaderboard.length) {
      return (
        <View style={styles.emptyCard}>
          ...
          <Text style={styles.emptyText}>Finish a few games to populate the stats view.</Text>
        </View>
      );
    }
    ...
    if (!selectedPlayer) {
      return (
        <View style={styles.emptyCard}>
          ...
          <Text style={styles.emptyText}>Choose a player to view their stats breakdown.</Text>
        </View>
      );
    }

    const accent = getPlayerColor(selectedPlayer.color);
    const selectedIndex = leaderboard.findIndex((p) => p.id === selectedPlayer.id);

    return (
      <View style={styles.playersList}>
        <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.selectorWrap}>
          {leaderboard.map((player) => {
            ...
            return (
              ...
              >
                <Text style={[styles.selectorTabText, active && styles.selectorTabTextActive]}>
                  {player.name}
                </Text>
                <View style={[styles.selectorTabUnderline, active && styles.selectorTabUnderlineActive]} />
              </Pressable>
            );
          })}
        </ScrollView>
        ...
        <View style={[styles.playerCard, { borderColor: `${accent}33` }]}>
          ...
          <View style={[styles.playerGlow, { backgroundColor: getGlowColor(selectedPlayer.color) }]} />
          ...
          <View style={styles.playerHeader}>
            <View style={styles.playerHeaderLeft}>
              ...
              />
              <View style={{ flex: 1 }}>
                <Text style={[styles.rankText, { color: accent }]}>#{selectedIndex + 1}</Text>
                <Text style={styles.playerName}>{selectedPlayer.name}</Text>
                <Text style={styles.playerMeta}>
                  {formatPercent(selectedPlayer.winRate)} win � {selectedPlayer.games} games � {selectedPlayer.avgPrestigePerGame.toFixed(1)} prestige/game
                </Text>
              </View>
            </View>

            <View style={styles.playerPrimaryValueWrap}>
              <Text style={styles.playerPrimaryValue}>{selectedPlayer.totalPrestige}</Text>
              <Text style={styles.playerPrimaryLabel}>prestige</Text>
            </View>
          </View>

          <PlayerSection title="Core">
            <StatPill label="Wins" value={selectedPlayer.wins} accent={COLORS.success} strong metricKey="wins" onInfoPress={() => goToDefinition('wins')} />
            <StatPill label="Direct" value={selectedPlayer.directPrestige} accent={COLORS.blueGlow} strong metricKey="directPrestige" onInfoPress={() => goToDefinition('directPrestige')} />
            <StatPill label="Assist In" value={selectedPlayer.assistPrestigeReceived.toFixed(1)} accent={COLORS.teal} strong metricKey="assistPrestigeReceived" onInfoPress={() => goToDefinition('assistPrestigeReceived')} />
            <StatPill label="Assist Out" value={selectedPlayer.assistPrestigeSent.toFixed(1)} accent={COLORS.purple} strong metricKey="assistPrestigeSent" onInfoPress={() => goToDefinition('assistPrestigeSent')} />

            <StatPill label="All Eff" value={selectedPlayer.allContractsEfficiency.toFixed(2)} accent={COLORS.success} metricKey="allContractsEfficiency" onInfoPress={() => goToDefinition('allContractsEfficiency')} />
            <StatPill label="Assist Eff" value={selectedPlayer.assistanceEfficiency.toFixed(2)} accent={COLORS.purple} metricKey="assistanceEfficiency" onInfoPress={() => goToDefinition('assistanceEfficiency')} />
            <StatPill label="Direct Eff" value={selectedPlayer.directEfficiency.toFixed(2)} accent={COLORS.blueGlow} metricKey="directEfficiency" onInfoPress={() => goToDefinition('directEfficiency')} />
            <StatPill label="Fail %" value={formatPercent(selectedPlayer.failureRate)} accent={COLORS.danger} metricKey="failureRate" onInfoPress={() => goToDefinition('failureRate')} />
          </PlayerSection>

          <PlayerSection title="Performance">
            <StatPill label="Consistency" value={selectedPlayer.consistencyScore.toFixed(2)} accent={COLORS.success} metricKey="consistencyScore" onInfoPress={() => goToDefinition('consistencyScore')} />
            <StatPill label="Clutch" value={formatPercent(selectedPlayer.clutchScore)} accent={COLORS.blue} metricKey="clutchScore" onInfoPress={() => goToDefinition('clutchScore')} />
            <StatPill label="Carry" value={formatPercent(selectedPlayer.carryFactor)} accent={COLORS.blueGlow} metricKey="carryFactor" onInfoPress={() => goToDefinition('carryFactor')} />
            <StatPill label="Momentum" value={formatSigned(selectedPlayer.momentum)} accent={COLORS.purple} metricKey="momentum" onInfoPress={() => goToDefinition('momentum')} />

            <StatPill label="Prestige / Turn" value={selectedPlayer.prestigePerTurn.toFixed(2)} accent={COLORS.teal} metricKey="prestigePerTurn" onInfoPress={() => goToDefinition('prestigePerTurn')} />
            <StatPill label="Early Lead %" value={formatPercent(selectedPlayer.earlyLeadRate)} accent={COLORS.blue} />
            <StatPill label="Late Lead %" value={formatPercent(selectedPlayer.lateLeadRate)} accent={COLORS.gold} />
            <StatPill label="Lead Conv" value={formatPercent(selectedPlayer.leadConversion)} accent={COLORS.success} metricKey="leadConversion" onInfoPress={() => goToDefinition('leadConversion')} />

            <StatPill label="Late Lead Conv" value={formatPercent(selectedPlayer.lateLeadConversion)} accent="#10B981" metricKey="lateLeadConversion" onInfoPress={() => goToDefinition('lateLeadConversion')} />
            <StatPill label="Objective Conv" value={formatPercent(selectedPlayer.objectiveConversionRate)} accent={COLORS.cyan} />
            <StatPill label="Support Conv" value={formatPercent(selectedPlayer.supportConversionRate)} accent={COLORS.purple} />
            <StatPill label="Avg Margin" value={formatSigned(selectedPlayer.avgPrestigeMarginPerGame)} accent={COLORS.blueGlow} />

            <StatPill label="Best Margin" value={formatSigned(selectedPlayer.bestPrestigeMargin)} accent={COLORS.blueGlow} />
            <StatPill label="Objectives / Game" value={selectedPlayer.objectivesPerGame.toFixed(2)} accent={COLORS.gold} />
            <StatPill label="Assists / Game" value={selectedPlayer.assistsGivenPerGame.toFixed(2)} accent={COLORS.teal} />
            <StatPill label="Assist In / Game" value={selectedPlayer.assistsReceivedPerGame.toFixed(2)} accent={COLORS.blueGlow} />
          </PlayerSection>

          <PlayerSection title="Context">
            <StatPill label="Opponent Str" value={selectedPlayer.opponentStrength.toFixed(2)} accent={COLORS.textSecondary} />
            <StatPill label="Tempo" value={selectedPlayer.tempoIndex.toFixed(2)} accent={COLORS.pink} metricKey="tempoIndex" onInfoPress={() => goToDefinition('tempoIndex')} />
            <StatPill label="Interaction" value={selectedPlayer.interactionIndex.toFixed(2)} accent={COLORS.cyan} />
            <StatPill label="Aggro" value={selectedPlayer.aggroIndex.toFixed(2)} accent={COLORS.danger} />
            <StatPill label="Seat ? Win Corr" value={formatCorrelation(selectedPlayer.turnOrderWinCorrelation)} accent={COLORS.success} />
            <StatPill label="Assist Count In" value={selectedPlayer.assistCountBySource} />
          </PlayerSection>
        </View>
      </View>
    );
  ...
  const renderCorrelationTab = () => (
    <View style={styles.card}>
      ...
      <View style={styles.subtabWrap}>
        <TabButton label="Global" active={activeCorrelationTab === 'global'} onPress={() => setActiveCorrelationTab('global')} />
        <TabButton label="Individual" active={activeCorrelationTab === 'individual'} onPress={() => setActiveCorrelationTab('individual')} />
        <TabButton label="Game" active={activeCorrelationTab === 'game'} onPress={() => setActiveCorrelationTab('game')} />
      </View>
      ...
      {activeCorrelationTab === 'individual' && (
        <View style={styles.list}>
            ...
            {leaderboard.map((player) => {
              ...
              return (
                <Pressable
                  ...
                  style={[ active && styles.selectorTabUnderlineActive]}
                >
                  <Text style={[styles.selectorTabText, active && styles.selectorTabTextActive]}>
                    {player.name}
                  </Text>
          ...
          {selectedCorrelationPlayer ? (
            <Text style={styles.subtitle}>
              What tends to drive wins specifically for {selectedCorrelationPlayer.name}.
            </Text>
          ) : null}
      ...
      {activeCorrelationTab === 'game' && (
        <View style={styles.list}>
          <Text style={styles.subtitle}>
            Match-condition signals showing what kinds of games favor a certain type of winner.
          </Text>
  ...
  const renderGamesTab = () => (
    <View style={styles.card}>
      ...
      <Text style={styles.title}>Game-Level View</Text>
      <Text style={styles.subtitle}>
      ...
      {selectedGame ? (
        <View style={styles.gamePanel}>
          ...
          <View style={styles.compactGrid}>
            <StatPill label="Margin" value={selectedGame.margin.toFixed(1)} accent={COLORS.blueGlow} strong />
            <StatPill label="Assists" value={selectedGame.assists} accent={COLORS.teal} strong />
            <StatPill label="Objectives" value={selectedGame.objectives} accent={COLORS.gold} strong />
            <StatPill label="Failures" value={selectedGame.failures} accent={COLORS.danger} strong />
          </View>

          <View style={styles.list}>
            {selectedGame.rows.map((row: any, index: number) => {
              const isWinner = row.playerId === selectedGame.winnerId;
              const player = leaderboard.find((p) => p.id === row.playerId);
              return (
                <View key={row.playerId} style={styles.gameRowCard}>
                  <View style={styles.gameRowIdentity}>
  <PlayerInitialBadge
    initials={player?.initials ?? (player?.name?.slice(0, 2)?.toUpperCase() ?? '?')}
    color={player?.color}
    size={26}
    fontSize={10}
  />
    ...
    <Text style={styles.gameRowMeta}>
      {isWinner ? 'Winner' : 'Participant'}
                  ...
                  <View style={styles.gameRowStats}>
                    <Text style={styles.gameRowStat}>P {row.prestige.toFixed(1)}</Text>
      ...
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        <View style={styles.heroCard}>
          ...
          <Text style={styles.heroSubtitle}>
            Condensed analytics, player trends, and winning signals in the same fleet-command visual system.
          </Text>
        </View>
        ...
        >
          ...
          <TabButton label="Insights" active={activeTab === 'correlations'} onPress={() => setActiveTab('correlations')} />
          <TabButton label="Games" active={activeTab === 'games'} onPress={() => setActiveTab('games')} />
        </ScrollView>
