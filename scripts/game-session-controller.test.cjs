...
// gameSessionController imports through the "@/" alias, which node's own module
// resolution cannot follow, so this test loads it through the shared shim.
...
assert.equal(
  candidate.nextRounds.length,
  2,
  "expected one main round plus one objective bonus round",
);
...
console.log("game-session-controller.test.cjs passed");
