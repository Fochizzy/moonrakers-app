...
function buildEloRows(
  ...
  const focusSeries =
    eloState.seriesPaths.find((series) => series.id === focusPlayerId) ??
    eloState.seriesPaths.find((series) => series.isFocused) ??
    eloState.seriesPaths[0] ??
    null;
  const compareSeries = comparePlayerId
    ? eloState.seriesPaths.find((series) => series.id === comparePlayerId) ?? null
    : null;
