-- Restores lifetime totals to statsScreen.players.detail.stats.
--
-- 20260525172608 emitted totalScore and totalPrestige there. 20260525210000 rebuilt
-- the same player_detail object and dropped them, but every consumer kept reading
-- them, so coalesce(..., 0) has been quietly serving 0 ever since:
--   * get_elo_screen leaderboard rows and summary -> Home leaderboard shows
--     "Score 0", "Total Prestige 0", "Prestige / Game 0.0" for every player, and
--     its score/prestige/avgPrestige sort options are no-ops.
--   * the elo snapshot builder writes the same zeros into eloProfile.
-- Sibling reads of 'games' and 'wins' survived, which is why only these two died.
--
-- Body is otherwise identical to 20260525210000.
--
-- Extends admin_refresh_analytics with contract efficiency, chaos index, and group meta.
-- Adds contractEfficiency, groupMeta, headToHead to statsScreen payload.
-- headToHead was previously only in insightsScreen.rivalries; now also in statsScreen.
...
declare
  ...
  avg_contracts_per_game numeric := 0;
  ...
  chaos_avg_spread numeric := 0;
  chaos_stddev_spread numeric := 0;
  optimal_player_count integer := null;
  contract_efficiency jsonb := '{}'::jsonb;
  group_meta jsonb := '{}'::jsonb;
  lifetime_score numeric := 0;
  lifetime_prestige numeric := 0;
  avg_score_per_game numeric := 0;
begin
  ...
  -- Lifetime totals over finished games only, matching how finished_game_count is scoped.
  select
    coalesce(sum(coalesce(gp.score, 0)), 0)::numeric,
    coalesce(sum(coalesce(gp.total_prestige, 0)), 0)::numeric,
    case
      when count(*) > 0
        then round(coalesce(sum(coalesce(gp.score, 0)), 0)::numeric / count(*), 1)
      else 0::numeric
    end
  into lifetime_score, lifetime_prestige, avg_score_per_game
  ...
  -- Consolidated participant query: signal stats + prestige sources + correlations + avg contracts per game
  select
    ...
    coalesce(round(avg(gp.contracts), 2), 0)::numeric
  into
    finished_game_count,
  ...
  -- Consolidated round query: pace + phases + consistency in a single game_rounds scan
  select
  ...
  -- Optimal player count: table size where this player has the highest win rate (min 2 games)
  select (x->>'playerCount')::int
  into optimal_player_count
  from jsonb_array_elements(player_count_split) as x
  where (x->>'games')::int >= 2
  order by (x->>'winRate')::numeric desc, (x->>'games')::int desc
  limit 1;
  ...
  -- Chaos index: stddev of prestige spread across this player's games
  select
    coalesce(round(avg(ga.prestige_spread), 1), 0)::numeric,
    coalesce(round(stddev_pop(ga.prestige_spread), 1), 0)::numeric
  into chaos_avg_spread, chaos_stddev_spread
  from public.game_participants as gp
  join public.games as g on g.id = gp.game_id and g.status = 'finished'
  join lateral (
    select (max(a.total_prestige) - min(a.total_prestige))::int as prestige_spread
  ...
  player_detail := jsonb_build_object(
    ...
    'stats', jsonb_build_object(
      ...
      'totalScore', lifetime_score,
      'totalPrestige', lifetime_prestige,
      'avgScore', avg_score_per_game));

  -- Contract efficiency section
  contract_efficiency := jsonb_build_object(
    'avgContractsPerGame', avg_contracts_per_game,
    'contractConversion', round(signal_contract_conversion * 100, 1),
    'abandonmentRate', round((1 - signal_contract_conversion) * 100, 1),
    'description', case when finished_game_count = 0 then 'No games yet.'
      else concat(
        'Avg ', avg_contracts_per_game::text, ' contracts/game. ',
        round(signal_contract_conversion * 100, 0)::int::text, '% conversion rate, ',
        round((1 - signal_contract_conversion) * 100, 0)::int::text, '% abandonment rate.')
      end);

  -- Group meta section
  group_meta := jsonb_build_object(
    'chaosIndex', chaos_stddev_spread,
    'avgSpread', chaos_avg_spread,
    'optimalPlayerCount', optimal_player_count,
    'playerCountSplit', player_count_split,
    'description', case when finished_game_count = 0 then 'No games yet.'
      else concat(
        'Avg prestige spread: ', chaos_avg_spread::text, '. ',
        'Chaos index (stddev): ', chaos_stddev_spread::text, '.',
        case when optimal_player_count is not null
          then concat(' Best win rate at ', optimal_player_count::text, '-player tables.')
          else '' end)
      end);
  ...
  analytics_payload := jsonb_build_object(
    ...
    'statsScreen', jsonb_build_object(
      ...
      'contractEfficiency', contract_efficiency,
...
-- Backfill all profiles with the updated analytics
do $$
declare p record;
begin
