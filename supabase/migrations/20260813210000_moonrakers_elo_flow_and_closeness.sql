...
-- Make ELO read the flow of a game and how close it was.
--
-- Two changes on top of the peak/prestige/end-score blend:
--
--   1. Flow replaces peak score. Peak only recorded the highest point a player
--      reached; it could not tell a wire-to-wire lead from a last-turn steal.
--      Flow measures each player's differential against the rest of the field
--      after every turn, then averages those standings with later turns
--      weighted more heavily than early ones (turn 1 carries weight 1, turn 2
--      weight 2, and so on). The opening still counts, just less than the
--      endgame. The win/loss result keeps its 60% share.
--
--   2. The rating swing now scales with how decisively the game was won. A game
--      settled by a single point of prestige barely proves anything and moves
--      ratings at 0.75x; a rout moves them at 1.25x. The margin is measured
--      between the winner and the runner-up, relative to the table average.
--
-- Every signal is now a differential against the field rather than a share of
-- it, so 0.5 means level with the average opponent in every case.

drop function if exists private.elo_field_share(numeric, numeric);
