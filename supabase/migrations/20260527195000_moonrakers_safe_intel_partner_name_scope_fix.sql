-- Fixes the remaining support-partner player_name scope ambiguity in the safe-intel fallback.
...
begin
  ...
  if moonrakers_intel is null or coalesce((moonrakers_intel->>'hasData')::boolean,false)=false then
    select count(*)::int
    ...
    if moonrakers_game_count < 3 then
      ...
      per_game_base as (
        ...
        left join public.game_rounds as gr
          on gr.participant_id = pg.participant_id
        group by pg.game_id
      ),
      objective_games as (
        select
      ...
      inferred_assist_events_count := assist_events_count;
      timed_assist_events_count := 0;
      ...
      timed_assist_events_label := '0 timed assists';
