...
-- #3: save_completed_game — idempotent via client_game_id
...
begin
  ...
  -- #3: Idempotency check — if client already submitted this game_id, return existing
...
-- #1 #2 #5: admin_refresh_analytics with personalBests, prestigePerContract, session cadence
...
declare
  ...
  -- #2: prestige per contract by phase
  ...
  pb_most_contracts  integer := 0; pb_perfect_wins integer := 0;
  pb_best_group_name text    := null;
  -- #5: session cadence
...
begin
  ...
  -- #1: Personal bests
  select
    coalesce(max(gp.total_prestige),0)::int, coalesce(max(gp.score),0)::numeric,
    coalesce(max(gp.contracts),0)::int,
    count(*) filter (where gp.failures=0 and gp.contracts>=3 and gp.is_winner)::int
  into pb_best_prestige, pb_best_score, pb_most_contracts, pb_perfect_wins
  from public.game_participants as gp
  join public.games as g on g.id=gp.game_id and g.status='finished'
  where gp.profile_id=target_profile_id;

  select grp.name into pb_best_group_name
  from public.game_participants as gp
  join public.games as g on g.id=gp.game_id and g.status='finished' and g.group_id is not null
  join public.groups as grp on grp.id=g.group_id
  where gp.profile_id=target_profile_id
  group by grp.id, grp.name having count(*)>=2
  order by count(*) filter (where gp.is_winner)::numeric/count(*) desc limit 1;

  -- Phase stats (round query) — #2 adds ppc_early/mid/late
  select
    ...
    -- #2: prestige per contract — total prestige / total contracts within each phase
    coalesce(round(sum(ep)::numeric/nullif(sum(ec),0),3),0)::numeric,
    coalesce(round(sum(mp)::numeric/nullif(sum(mc),0),3),0)::numeric,
    coalesce(round(sum(lp)::numeric/nullif(sum(lc),0),3),0)::numeric
  into
  ...
  -- Closing strength + prestige per turn
  select
  ...
  -- #2: roundPhaseStats now includes prestigePerContract per phase
  round_phase_stats:=jsonb_build_object(
    'early',jsonb_build_object('label','Early (rounds 0-6)',  'avgPrestigePerRound',rps_prestige_early,'avgContractsPerRound',rps_contracts_early,'avgFailuresPerRound',rps_failures_early,'avgObjectivesPerRound',rps_objectives_early,'prestigePerContract',rps_ppc_early),
    'mid',  jsonb_build_object('label','Mid (rounds 7-14)',   'avgPrestigePerRound',rps_prestige_mid,  'avgContractsPerRound',rps_contracts_mid,  'avgFailuresPerRound',rps_failures_mid,  'avgObjectivesPerRound',rps_objectives_mid,  'prestigePerContract',rps_ppc_mid),
    'late', jsonb_build_object('label','Late (rounds 15+)',   'avgPrestigePerRound',rps_prestige_late, 'avgContractsPerRound',rps_contracts_late, 'avgFailuresPerRound',rps_failures_late, 'avgObjectivesPerRound',rps_objectives_late, 'prestigePerContract',rps_ppc_late));
  ...
  if finished_game_count>=3 then
    ...
    select start_order, appearances, round(wins::numeric/appearances,3)
    into seat_best_order, seat_best_apps, seat_best_win_pct
  ...
  -- #5: Session cadence (distinct play days, gaps between them)
  with sessions as (
    select date(coalesce(g.finished_at, g.created_at)) as sd
    from public.games as g
    join public.game_participants as gp on gp.game_id=g.id and gp.profile_id=target_profile_id
    where g.status='finished'
    group by date(coalesce(g.finished_at, g.created_at))
  ),
  ...
  into sc_session_count, sc_avg_gap, sc_longest_gap, sc_last_session_date
  from gaps;
  ...
  if finished_game_count>=2 then
    ...
    select coalesce((select corr(acs.avg_gap_to_target::double precision,acs.victory) from assist_context_samples as acs where acs.avg_gap_to_target is not null)::numeric,0), coalesce((select corr(acs.avg_gap_to_leader::double precision,acs.victory) from assist_context_samples as acs where acs.avg_gap_to_leader is not null)::numeric,0), coalesce((select corr(acs.assists_at_six_plus::double precision,acs.victory) from assist_context_samples as acs)::numeric,0), coalesce((select corr(acs.assists_over_five_behind::double precision,acs.victory) from assist_context_samples as acs)::numeric,0), coalesce((select corr(acs.assist_prestige_gained::double precision,acs.victory) from assist_context_samples as acs)::numeric,0), coalesce((select corr(acs.avg_gap_to_target::double precision,acs.victory) from assist_context_samples as acs where acs.profile_id=target_profile_id and acs.avg_gap_to_target is not null)::numeric,0), coalesce((select corr(acs.avg_gap_to_leader::double precision,acs.victory) from assist_context_samples as acs where acs.profile_id=target_profile_id and acs.avg_gap_to_leader is not null)::numeric,0), coalesce((select corr(acs.assists_at_six_plus::double precision,acs.victory) from assist_context_samples as acs where acs.profile_id=target_profile_id)::numeric,0), coalesce((select corr(acs.assists_over_five_behind::double precision,acs.victory) from assist_context_samples as acs where acs.profile_id=target_profile_id)::numeric,0), coalesce((select corr(acs.assist_prestige_gained::double precision,acs.victory) from assist_context_samples as acs where acs.profile_id=target_profile_id)::numeric,0)
    into macro_assist_target_gap,macro_assist_leader_gap,macro_assists_at_six_plus,macro_assists_over5_behind,macro_assist_prestige_gained, personal_assist_target_gap,personal_assist_leader_gap,personal_assists_at_six_plus,personal_assists_over5_behind,personal_assist_prestige_gained;
  ...
  analytics_payload:=jsonb_build_object(
    ...
    -- #1: analyticsHome now includes personalBests + sessionCadence (#5)
    ...
    'statsScreen',jsonb_build_object('generatedAt',generated_at,
      ...
      'paceProfile',jsonb_build_object('avgFirstHalf',pace_avg_first_half,'avgSecondHalf',pace_avg_second_half,'avgLateDelta',pace_avg_late_delta,'avgFirstHalfWin',pace_avg_first_half_win,'avgFirstHalfLose',pace_avg_first_half_lose,'avgOpeningTurns',pace_avg_opening_turns,'avgClosingTurns',pace_avg_closing_turns,'avgClosingTurnsWin',pace_avg_closing_turns_win,'avgClosingTurnsLoss',pace_avg_closing_turns_loss,'description',case when finished_game_count=0 then 'No games yet.' else concat('Avg prestige: ',pace_avg_first_half::text,' (first half) -> ',pace_avg_second_half::text,' (second half). Final 3 turns avg: ',pace_avg_closing_turns::text,' (',case when pace_avg_closing_turns>pace_avg_opening_turns+0.5 then 'strong closer' when pace_avg_opening_turns>pace_avg_closing_turns+0.5 then 'strong opener' else 'steady' end,').') end),
    ...
    'insightsScreen',jsonb_build_object('generatedAt',generated_at,
      ...
      'correlations',jsonb_build_object('summary',case when finished_game_count<2 then 'Need at least 2 games for insights.' else 'Outcome signals derived from your finished games.' end,'personal',personal_payload,'pairing',pairing_payload,'macro',macro_payload,'synergyPairs',synergy_payload,'players','[]'::jsonb,'items',correlations_items,'selectedKey',null,'winLoseSplit',correlations_items)));
  ...
  -- Group rollup — #5 adds sessionWarmup lateral
  insert into public.group_stats_rollups (group_id,payload,updated_at)
  select g.id,
    jsonb_build_object('groupId',g.id,'name',g.name,'gamesPlayed',coalesce(gc.game_count,0),'lastGameId',gc.last_game_id,'memberCount',coalesce(mc.member_count,0),'memberStats',coalesce(ms.stats,'[]'::jsonb),'topWinner',ms.top_winner_name,'seatBias',coalesce(sb.stats,'[]'::jsonb),
      -- #5: group-level session warmup pattern
      'sessionWarmup',jsonb_build_object('pattern',coalesce(sw.pattern,'Consistent'),'game1WinRate',coalesce(sw.g1wr,0),'game3PlusWinRate',coalesce(sw.g3wr,0))),
    now()
  ...
  left join lateral (
    select coalesce(jsonb_agg(jsonb_build_object('profileId',p.id,'displayName',coalesce(nullif(p.display_name,''),p.player_name,'Player'),'games',ms_inner.games,'wins',ms_inner.wins,'winRate',case when ms_inner.games>0 then round(ms_inner.wins::numeric/ms_inner.games,3) else 0 end,'avgPrestige',ms_inner.avg_prestige,'avgScore',ms_inner.avg_score) order by ms_inner.wins desc,ms_inner.avg_prestige desc),'[]'::jsonb) as stats,
      (select coalesce(nullif(p2.display_name,''),p2.player_name) from public.profiles as p2 where p2.id=(select gp2.profile_id from public.game_participants as gp2 join public.games as gg2 on gg2.id=gp2.game_id and gg2.group_id=g.id and gg2.status='finished' where gp2.profile_id is not null group by gp2.profile_id order by count(*) filter (where gp2.is_winner) desc,count(*) desc limit 1)) as top_winner_name
    from (select gp.profile_id,count(distinct gp.game_id)::int as games,count(*) filter (where gp.is_winner)::int as wins,round(avg(gp.total_prestige),1) as avg_prestige,round(avg(gp.score),1) as avg_score from public.game_participants as gp join public.games as gg on gg.id=gp.game_id and gg.group_id=g.id and gg.status='finished' where gp.profile_id is not null group by gp.profile_id) as ms_inner
    join public.profiles as p on p.id=ms_inner.profile_id
  ) as ms on true
  left join lateral (
    select coalesce(jsonb_agg(jsonb_build_object('seat',sd.start_order,'appearances',sd.appearances,'wins',sd.wins,'winRate',case when sd.appearances>0 then round(sd.wins::numeric/sd.appearances,3) else 0 end) order by sd.start_order),'[]'::jsonb) as stats
    from (select gp.start_order,count(*)::int as appearances,count(*) filter (where gp.is_winner)::int as wins from public.game_participants as gp join public.games as gg on gg.id=gp.game_id and gg.group_id=g.id and gg.status='finished' where gp.profile_id is not null group by gp.start_order) as sd
  ) as sb on true
  left join lateral (
    with grp_sessions as (
      select gp.profile_id,gp.is_winner,
        row_number() over (partition by date(coalesce(gg.finished_at,gg.created_at)) order by coalesce(gg.finished_at,gg.created_at),gg.id) as game_in_session
      from public.game_participants as gp
      join public.games as gg on gg.id=gp.game_id and gg.group_id=g.id and gg.status='finished'
      where gp.profile_id is not null
    )
    select
      round(count(*) filter (where is_winner and game_in_session=1)::numeric/nullif(count(*) filter (where game_in_session=1),0),3)   as g1wr,
      round(count(*) filter (where is_winner and game_in_session>=3)::numeric/nullif(count(*) filter (where game_in_session>=3),0),3) as g3wr,
      case when round(count(*) filter (where is_winner and game_in_session>=3)::numeric/nullif(count(*) filter (where game_in_session>=3),0),3)
                > round(count(*) filter (where is_winner and game_in_session=1)::numeric/nullif(count(*) filter (where game_in_session=1),0),3)+0.15
           then 'Warms up' else 'Consistent' end as pattern
    from grp_sessions
  ) as sw on true
