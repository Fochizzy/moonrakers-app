...
-- (qual=can_read_game). Since policies are PERMISSIVE (OR logic), the TRUE policy
-- unconditionally matches every row, making can_read_game unreachable.
-- Removing the dead policies eliminates the per-row can_read_game function calls.
drop policy if exists game_participants_select_participants on public.game_participants;
drop policy if exists game_rounds_select_participants on public.game_rounds;
drop policy if exists games_select_participants on public.games;


-- #4 + #5: Update admin_refresh_analytics
-- #4: Add avgObjectivesPerRound to each phase in roundPhaseStats (free from existing scan)
-- #5: Add WHERE deleted_at IS NULL to registered_player_count
...
declare
  ...
  rps_objectives_early numeric := 0;  -- #4
  rps_objectives_mid numeric := 0;    -- #4
  rps_objectives_late numeric := 0;   -- #4
...
begin
  ...
  -- #5: only count non-deleted profiles
  ...
  -- #4: objective_count per phase added (eo/mo/lo) at zero extra I/O cost.
  select
    ...
    -- #4: avg objectives per round by phase
    coalesce(round(sum(eo)::numeric / nullif(sum(er), 0), 3), 0)::numeric,
    coalesce(round(sum(mo)::numeric / nullif(sum(mr), 0), 3), 0)::numeric,
    coalesce(round(sum(lo)::numeric / nullif(sum(lr), 0), 3), 0)::numeric,
  ...
  -- #4: avgObjectivesPerRound added to each phase
  round_phase_stats := jsonb_build_object(
    'early', jsonb_build_object('label', 'Early (rounds 0-6)',
      'avgPrestigePerRound', rps_prestige_early, 'avgContractsPerRound', rps_contracts_early,
      'avgFailuresPerRound', rps_failures_early, 'avgObjectivesPerRound', rps_objectives_early),
    'mid',   jsonb_build_object('label', 'Mid (rounds 7-14)',
      'avgPrestigePerRound', rps_prestige_mid,   'avgContractsPerRound', rps_contracts_mid,
      'avgFailuresPerRound', rps_failures_mid,   'avgObjectivesPerRound', rps_objectives_mid),
    'late',  jsonb_build_object('label', 'Late (rounds 15+)',
      'avgPrestigePerRound', rps_prestige_late,  'avgContractsPerRound', rps_contracts_late,
      'avgFailuresPerRound', rps_failures_late,  'avgObjectivesPerRound', rps_objectives_late));
  ...
  with game_sessions as (
    select gp.is_winner, gp.total_prestige,
  ...
  position_agg as (
    select pos, count(*)::int as appearances, count(*) filter (where is_winner)::int as wins, round(avg(total_prestige),1) as avg_prestige
    from game_sessions group by pos
  )
  select
    coalesce(jsonb_agg(jsonb_build_object('gameNumber',pos,'appearances',appearances,'wins',wins,'winRate',case when appearances>0 then round(wins::numeric/appearances,3) else 0::numeric end,'avgPrestige',avg_prestige) order by pos),'[]'::jsonb),
  ...
  session_profile := jsonb_build_object('items',session_profile_items,'tendency',session_tendency,'earlyWinRate',session_early_wr,'lateWinRate',session_late_wr,
    'description',case when finished_game_count=0 then 'No games yet.' when session_tendency='Warms up' then concat('Performs better as sessions progress (game 1-2: ',round(session_early_wr*100,0)::int::text,'% -> game 3+: ',round(session_late_wr*100,0)::int::text,'% win rate).') when session_tendency='Fades late' then concat('Performance declines through sessions (game 1-2: ',round(session_early_wr*100,0)::int::text,'% -> game 3+: ',round(session_late_wr*100,0)::int::text,'% win rate).') else 'Win rate is consistent across games within a session.' end);
  ...
  analytics_payload := jsonb_build_object(
    ...
    'analyticsHome', jsonb_build_object(
      ...
      'cards', jsonb_build_array(
        jsonb_build_object('key','registered-players','title','Registered players','value',registered_player_count,'description','Active players available to analytics.'),
        ...
        jsonb_build_object('key','days-since-last-game','title','Days since last game','value',coalesce(days_since_last_game::text,'-'),'description','Calendar days since this player''s most recent game.'))),
    ...
    'insightsScreen', jsonb_build_object(
      ...
      'correlations', jsonb_build_object('summary',case when finished_game_count<3 then 'Need at least 3 games for meaningful correlation analysis.' else 'Win/loss correlations across objectives, assists, and failures.' end,'items',correlations_items,'selectedKey',null)));
