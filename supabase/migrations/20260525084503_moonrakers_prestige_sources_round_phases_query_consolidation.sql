...
-- #2: Prestige source fingerprinting (direct / assist-received / objective, with win/loss splits)
-- #5: Round-phase productivity (early 0-6, mid 7-14, late 15+) — prestige/contracts/failures per round
-- #6: Query consolidation
--     OLD: ~12 sequential SELECT INTO statements
--     NEW: one combined participant query (merges signal stats + prestige sources + correlations)
--          one combined round query (merges pace profile + round phase stats — single game_rounds scan)
--     Net: ~8 queries instead of ~12 for a 4-player game save; game_rounds read exactly once
...
declare
  ...
  -- global counts (still separate — different tables, no profile filter)
  registered_player_count integer := 0;
  player_row_count integer := 0;
  days_since_last_game integer := null;
  -- ── consolidated participant query outputs ────────────────────────────────
  ...
  -- correlation inputs (reuse ps_avg_objective_win/lose; still need assist/failure splits)
  ...
  -- ── consolidated round query outputs ─────────────────────────────────────
  -- pace profile
  ...
  -- #5 round phase stats (avg per round in phase)
...
begin
  ...
  -- ── 2. Global counts (3 separate tables, kept as individual queries) ──────
  ...
  -- ── 3. CONSOLIDATED participant query (#6) ────────────────────────────────
  -- Replaces what were previously three separate SELECT INTO statements:
  --   a) finished_game_count
  --   b) signal inputs (win count, avg assists, avg failures, contract conversion)
  --   c) correlation inputs (avg stats filtered by win/loss)
  -- Also adds prestige source fingerprinting (#2) at zero extra I/O cost.
  select
    ...
    -- #2 prestige sources when winning
    ...
    -- #2 prestige sources when losing
    ...
    -- correlation win/loss splits (assist and failure — objective reuses ps_avg_objective_win/lose)
  ...
  -- Derive from the combined results (no extra queries needed)
  ...
  -- ── 4. CONSOLIDATED round query (#5 + #6) ────────────────────────────────
  -- Replaces the pace profile query AND adds round-phase stats, all in a single
  -- scan of game_rounds. Previously pace required one lateral join over game_rounds;
  -- round phases would have required a second. Now both are computed together.
  select
    -- pace profile (first half vs second half of each game)
    ...
    -- #5 round phase: avg prestige per round in each phase (pooled across all games)
    ...
    -- #5 round phase: contracts per round
    ...
    -- #5 round phase: failures per round
  ...
  from (
    -- One row per participant per game; lateral supplies the midpoint round index.
    select
  ...
  -- ── 5. Build #2 prestige sources payload ──────────────────────────────────
  prestige_sources := jsonb_build_object(
    'avgDirect',          ps_avg_direct,
    'avgAssistReceived',  ps_avg_assist_recv,
    'avgObjective',       ps_avg_objective,
    ...
    'whenWin', jsonb_build_object(
      'avgDirect',         ps_avg_direct_win,
      'avgAssistReceived', ps_avg_assist_recv_win,
      'avgObjective',      ps_avg_objective_win
    ),
    'whenLose', jsonb_build_object(
      'avgDirect',         ps_avg_direct_lose,
      'avgAssistReceived', ps_avg_assist_recv_lose,
      'avgObjective',      ps_avg_objective_lose
    ),
    'description', case
  ...
  -- ── 6. Build #5 round phase stats payload ────────────────────────────────
  round_phase_stats := jsonb_build_object(
    'early', jsonb_build_object(
      'label', 'Early (rounds 0–6)',
      'avgPrestigePerRound',  rps_prestige_early,
      'avgContractsPerRound', rps_contracts_early,
      'avgFailuresPerRound',  rps_failures_early
    ),
    'mid', jsonb_build_object(
      'label', 'Mid (rounds 7–14)',
      'avgPrestigePerRound',  rps_prestige_mid,
      'avgContractsPerRound', rps_contracts_mid,
      'avgFailuresPerRound',  rps_failures_mid
    ),
    'late', jsonb_build_object(
      'label', 'Late (rounds 15+)',
      'avgPrestigePerRound',  rps_prestige_late,
      'avgContractsPerRound', rps_contracts_late,
      'avgFailuresPerRound',  rps_failures_late
    )
  );

  -- ── 7. Streaks (window function — must stay separate) ─────────────────────
  select
  ...
  if finished_game_count >= 3 then
    ...
    if streak_current_len >= 3 then
      if streak_current_is_win then
        top_signals := top_signals || jsonb_build_array(jsonb_build_object(
          'key', 'win-streak', 'label', concat(streak_current_len::text, '-game win streak'),
          'value', concat('W×', streak_current_len::text), 'tone', 'green'));
      else
        top_signals := top_signals || jsonb_build_array(jsonb_build_object(
          'key', 'loss-streak', 'label', concat(streak_current_len::text, '-game losing streak'),
  ...
  if finished_game_count >= 3 then
    correlations_items := jsonb_build_array(
      jsonb_build_object(
        'key', 'objectives-vs-wins', 'label', 'Objective prestige',
        'whenWin', corr_avg_obj_win, 'whenLose', corr_avg_obj_lose,
        ...
        'description', case
          ...
          else 'Objective prestige does not meaningfully differ between wins and losses' end),
      jsonb_build_object(
        'key', 'assists-vs-wins', 'label', 'Assists',
        'whenWin', corr_avg_assists_win, 'whenLose', corr_avg_assists_lose,
        ...
        'description', case
          ...
          else 'Assist volume is similar across wins and losses' end),
      jsonb_build_object(
        'key', 'failures-vs-wins', 'label', 'Failures',
        'whenWin', corr_avg_failures_win, 'whenLose', corr_avg_failures_lose,
        ...
        'description', case
          ...
          else 'Failure count does not meaningfully differ between wins and losses' end)
    );
  end if;
  ...
  select coalesce(jsonb_agg(
    jsonb_build_object(
      ...
      'gamesTogether', games_together, 'wins', wins, 'losses', losses,
  ...
  select coalesce(jsonb_agg(
    jsonb_build_object(
      'position', start_order, 'appearances', appearances, 'wins', wins,
  ...
  into position_stats
  from (
    select gp.start_order, count(*)::int as appearances,
  ...
  select coalesce(jsonb_agg(
    jsonb_build_object(
      'gameId', game_id, 'finishedAt', finished_at, 'groupName', group_name,
      'playerCount', player_count, 'isWinner', is_winner, 'prestige', total_prestige,
      'prestigeSpread', prestige_spread, 'winnerName', winner_name,
  ...
  from (
    select g.id as game_id, coalesce(g.finished_at, g.created_at) as finished_at,
      g.group_name_snapshot as group_name, game_agg.player_count, game_agg.prestige_spread,
      gp.is_winner, gp.total_prestige, gp.assists, gp.failures, gp.contracts,
    ...
    join lateral (
      ...
      from public.game_participants as agg_gp where agg_gp.game_id = g.id
    ) as game_agg on true
    where gp.profile_id = target_profile_id
  ) as game_data;

  -- ── 13. Chart / player setup (unchanged) ──────────────────────────────────
  ...
  player_options := jsonb_build_array(jsonb_build_object(
    ...
    'playerName', target_player_name, 'displayName', target_display_name));
  ...
  chart_rollups := jsonb_build_object(
    'default', jsonb_build_object('chartKey', 'default', 'generatedAt', generated_at, 'title', 'Analytics chart', 'subtitle', 'Server-authored placeholder dataset.', 'emptyState', jsonb_build_object('title', 'No chart data yet', 'description', 'Finish at least one tracked game to populate this chart.'), 'data', base_chart_data),
    'elo', jsonb_build_object('chartKey', 'elo', 'generatedAt', generated_at, 'title', 'Elo trend', 'subtitle', 'Server-authored placeholder dataset for Elo.', 'emptyState', jsonb_build_object('title', 'No Elo history yet', 'description', 'Finish at least one tracked game to populate Elo history.'), 'data', jsonb_build_object('games', '[]'::jsonb, 'players', '[]'::jsonb)),
    'prestige', jsonb_build_object('chartKey', 'prestige', 'generatedAt', generated_at, 'title', 'Prestige totals', 'subtitle', 'Server-authored placeholder dataset for prestige.', 'emptyState', jsonb_build_object('title', 'No prestige totals yet', 'description', 'Finish at least one tracked game to populate prestige totals.'), 'data', base_chart_data),
    'assists', jsonb_build_object('chartKey', 'assists', 'generatedAt', generated_at, 'title', 'Assist volume', 'subtitle', 'Server-authored placeholder dataset for assists.', 'emptyState', jsonb_build_object('title', 'No assist history yet', 'description', 'Finish at least one tracked game to populate assist history.'), 'data', base_chart_data),
    'radar', jsonb_build_object('chartKey', 'radar', 'generatedAt', generated_at, 'title', 'Radar comparison', 'subtitle', 'Server-authored placeholder dataset for radar comparisons.', 'emptyState', jsonb_build_object('title', 'No radar comparison yet', 'description', 'Finish at least one tracked game to populate radar comparisons.'), 'data', jsonb_build_object('primary', null, 'comparison', null, 'labels', '[]'::jsonb)),
    'relationship_graph', jsonb_build_object('chartKey', 'relationship_graph', 'generatedAt', generated_at, 'title', 'Relationship graph', 'subtitle', 'Server-authored placeholder dataset for player relationships.', 'emptyState', jsonb_build_object('title', 'No relationship graph yet', 'description', 'Finish at least one tracked game to populate relationship links.'), 'data', jsonb_build_object('nodes', '[]'::jsonb, 'edges', '[]'::jsonb, 'scopedPlayerIds', '[]'::jsonb, 'exactScopePlayerIds', '[]'::jsonb, 'mode', 'network')),
    'line_chart', jsonb_build_object('chartKey', 'line_chart', 'generatedAt', generated_at, 'title', 'Line chart', 'subtitle', 'Server-authored placeholder dataset for line charts.', 'emptyState', jsonb_build_object('title', 'No line chart yet', 'description', 'Finish at least one tracked game to populate line-chart history.'), 'data', jsonb_build_object('data', '[]'::jsonb, 'players', '[]'::jsonb, 'statKey', 'totalPrestige', 'selectedPlayerIds', '[]'::jsonb, 'scopedPlayerIds', '[]'::jsonb, 'mode', 'raw')),
    'line', jsonb_build_object('chartKey', 'line', 'generatedAt', generated_at, 'title', 'Line chart', 'subtitle', 'Server-authored placeholder dataset for line charts.', 'emptyState', jsonb_build_object('title', 'No line chart yet', 'description', 'Finish at least one tracked game to populate line-chart history.'), 'data', jsonb_build_object('data', '[]'::jsonb, 'players', '[]'::jsonb, 'statKey', 'totalPrestige', 'selectedPlayerIds', '[]'::jsonb, 'scopedPlayerIds', '[]'::jsonb, 'mode', 'raw')),
    'multi_line_chart', jsonb_build_object('chartKey', 'multi_line_chart', 'generatedAt', generated_at, 'title', 'Multi-line chart', 'subtitle', 'Server-authored placeholder dataset for multi-line charts.', 'emptyState', jsonb_build_object('title', 'No multi-line chart yet', 'description', 'Finish at least one tracked game to populate multi-line history.'), 'data', jsonb_build_object('data', '[]'::jsonb, 'players', '[]'::jsonb, 'statKey', 'totalPrestige', 'selectedPlayerIds', '[]'::jsonb, 'scopedPlayerIds', '[]'::jsonb, 'mode', 'raw')),
    'multi-line-chart', jsonb_build_object('chartKey', 'multi-line-chart', 'generatedAt', generated_at, 'title', 'Multi-line chart', 'subtitle', 'Server-authored placeholder dataset for multi-line charts.', 'emptyState', jsonb_build_object('title', 'No multi-line chart yet', 'description', 'Finish at least one tracked game to populate multi-line history.'), 'data', jsonb_build_object('data', '[]'::jsonb, 'players', '[]'::jsonb, 'statKey', 'totalPrestige', 'selectedPlayerIds', '[]'::jsonb, 'scopedPlayerIds', '[]'::jsonb, 'mode', 'raw')),
    'multi-line', jsonb_build_object('chartKey', 'multi-line', 'generatedAt', generated_at, 'title', 'Multi-line chart', 'subtitle', 'Server-authored placeholder dataset for multi-line charts.', 'emptyState', jsonb_build_object('title', 'No multi-line chart yet', 'description', 'Finish at least one tracked game to populate multi-line history.'), 'data', jsonb_build_object('data', '[]'::jsonb, 'players', '[]'::jsonb, 'statKey', 'totalPrestige', 'selectedPlayerIds', '[]'::jsonb, 'scopedPlayerIds', '[]'::jsonb, 'mode', 'raw')),
    'prestige_over_time', jsonb_build_object('chartKey', 'prestige_over_time', 'generatedAt', generated_at, 'title', 'Prestige over time', 'subtitle', 'Server-authored placeholder dataset for prestige-over-time charts.', 'emptyState', jsonb_build_object('title', 'No prestige-over-time chart yet', 'description', 'Finish at least one tracked game to populate prestige-over-time history.'), 'data', jsonb_build_object('data', '[]'::jsonb, 'players', '[]'::jsonb, 'statKey', 'totalPrestige', 'selectedPlayerIds', '[]'::jsonb, 'scopedPlayerIds', '[]'::jsonb, 'mode', 'raw')),
    'bar_chart', jsonb_build_object('chartKey', 'bar_chart', 'generatedAt', generated_at, 'title', 'Bar chart', 'subtitle', 'Server-authored placeholder dataset for bar charts.', 'emptyState', jsonb_build_object('title', 'No bar chart yet', 'description', 'Finish at least one tracked game to populate bar-chart comparisons.'), 'data', jsonb_build_object('data', '[]'::jsonb, 'players', '[]'::jsonb, 'statKey', 'totalPrestige', 'scopedPlayerIds', '[]'::jsonb)),
    'bar', jsonb_build_object('chartKey', 'bar', 'generatedAt', generated_at, 'title', 'Bar chart', 'subtitle', 'Server-authored placeholder dataset for bar charts.', 'emptyState', jsonb_build_object('title', 'No bar chart yet', 'description', 'Finish at least one tracked game to populate bar-chart comparisons.'), 'data', jsonb_build_object('data', '[]'::jsonb, 'players', '[]'::jsonb, 'statKey', 'totalPrestige', 'scopedPlayerIds', '[]'::jsonb)),
    'bump_chart', jsonb_build_object('chartKey', 'bump_chart', 'generatedAt', generated_at, 'title', 'Bump chart', 'subtitle', 'Server-authored placeholder dataset for bump charts.', 'emptyState', jsonb_build_object('title', 'No bump chart yet', 'description', 'Finish at least one tracked game to populate bump-chart rankings.'), 'data', jsonb_build_object('data', '[]'::jsonb, 'players', '[]'::jsonb, 'statKey', 'totalPrestige', 'selectedPlayerIds', '[]'::jsonb)),
    'consistency_band', jsonb_build_object('chartKey', 'consistency_band', 'generatedAt', generated_at, 'title', 'Consistency band', 'subtitle', 'Server-authored placeholder dataset for consistency bands.', 'emptyState', jsonb_build_object('title', 'No consistency band yet', 'description', 'Finish at least one tracked game to populate consistency-band trends.'), 'data', jsonb_build_object('data', '[]'::jsonb, 'players', '[]'::jsonb, 'statKey', 'totalPrestige', 'selectedPlayerIds', '[]'::jsonb, 'scopedPlayerIds', '[]'::jsonb)),
    'heatmap', jsonb_build_object('chartKey', 'heatmap', 'generatedAt', generated_at, 'title', 'Heatmap', 'subtitle', 'Server-authored placeholder dataset for heatmaps.', 'emptyState', jsonb_build_object('title', 'No heatmap yet', 'description', 'Finish at least one tracked game to populate heatmap data.'), 'data', jsonb_build_object('data', '[]'::jsonb, 'players', '[]'::jsonb, 'statKey', 'totalPrestige', 'scopedPlayerIds', '[]'::jsonb)),
    'efficiency_failure_scatter', jsonb_build_object('chartKey', 'efficiency_failure_scatter', 'generatedAt', generated_at, 'title', 'Efficiency scatter', 'subtitle', 'Server-authored placeholder dataset for efficiency-versus-failure scatter plots.', 'emptyState', jsonb_build_object('title', 'No efficiency scatter yet', 'description', 'Finish at least one tracked game to populate efficiency scatter data.'), 'data', jsonb_build_object('data', '[]'::jsonb, 'players', '[]'::jsonb, 'scopedPlayerIds', '[]'::jsonb)),
    'replay_chart', jsonb_build_object('chartKey', 'replay_chart', 'generatedAt', generated_at, 'title', 'Replay chart', 'subtitle', 'Server-authored placeholder dataset for replay charts.', 'emptyState', jsonb_build_object('title', 'No replay chart yet', 'description', 'Finish at least one tracked game to populate replay timelines.'), 'data', jsonb_build_object('replay', '[]'::jsonb, 'players', '[]'::jsonb, 'statKey', 'totalPrestige', 'selectedGameId', null)),
    'rivalry_graph', jsonb_build_object('chartKey', 'rivalry_graph', 'generatedAt', generated_at, 'title', 'Rivalry graph', 'subtitle', 'Server-authored placeholder dataset for rivalry graphs.', 'emptyState', jsonb_build_object('title', 'No rivalry graph yet', 'description', 'Finish at least one tracked game to populate rivalry data.'), 'data', jsonb_build_object('games', '[]'::jsonb, 'players', '[]'::jsonb, 'playerId', null)),
    'head_to_head', jsonb_build_object('chartKey', 'head_to_head', 'generatedAt', generated_at, 'title', 'Head-to-head', 'subtitle', 'Server-authored placeholder dataset for head-to-head comparisons.', 'emptyState', jsonb_build_object('title', 'No head-to-head data yet', 'description', 'Finish at least one tracked game to populate head-to-head comparisons.'), 'data', jsonb_build_object('games', '[]'::jsonb, 'players', '[]'::jsonb, 'playerId', null, 'compareId', null)),
    'sparkline', jsonb_build_object('chartKey', 'sparkline', 'generatedAt', generated_at, 'title', 'Sparkline', 'subtitle', 'Server-authored placeholder dataset for sparkline summaries.', 'emptyState', jsonb_build_object('title', 'No sparkline data yet', 'description', 'Finish at least one tracked game to populate sparkline summaries.'), 'data', jsonb_build_object('data', '[]'::jsonb, 'comparisonData', '[]'::jsonb, 'metricKey', 'totalPrestige', 'primaryLabel', null, 'comparisonLabel', null)),
    'stacked_bar_chart', jsonb_build_object('chartKey', 'stacked_bar_chart', 'generatedAt', generated_at, 'title', 'Stacked bar chart', 'subtitle', 'Server-authored placeholder dataset for stacked bar charts.', 'emptyState', jsonb_build_object('title', 'No stacked bar chart yet', 'description', 'Finish at least one tracked game to populate stacked-bar comparisons.'), 'data', jsonb_build_object('data', '[]'::jsonb, 'metricDataMap', '{}'::jsonb, 'metricOptions', '[]'::jsonb, 'activeMetricKey', 'totalPrestige', 'selectedPlayerIds', '[]'::jsonb)),
    'compare', jsonb_build_object('chartKey', 'compare', 'generatedAt', generated_at, 'title', 'Compare players', 'subtitle', 'Server-authored placeholder dataset for compare views.', 'emptyState', jsonb_build_object('title', 'No compare data yet', 'description', 'Finish at least one tracked game to populate player comparisons.'), 'data', jsonb_build_object('focusPlayerId', null, 'comparePlayerId', null, 'scopedPlayerIds', '[]'::jsonb, 'rows', '[]'::jsonb))
  );

  -- ── 14. Build analytics payload ───────────────────────────────────────────
  analytics_payload := jsonb_build_object(
    ...
    'statsScreen', jsonb_build_object(
      ...
      'overview', jsonb_build_object(
        'hero', jsonb_build_object(
          ...
          'takeaway', case
            ...
            else 'No finished games are available for this profile yet.' end,
      ...
      'paceProfile', jsonb_build_object(
        ...
        'description', case
          when finished_game_count = 0 then 'No games yet.'
          else concat('Avg prestige: ', pace_avg_first_half::text, ' (first half) → ', pace_avg_second_half::text, ' (second half), Δ +', pace_avg_late_delta::text, '.') end
      ),
      -- #5 round phase stats
      'roundPhaseStats', round_phase_stats,
      'playstyle', jsonb_build_object(
        'summary', case
          when finished_game_count > 0 then 'Playstyle data available. See prestigeSources, paceProfile, roundPhaseStats, correlations, and positionStats.'
          else 'Finish at least one tracked game to populate player-specific playstyle insights.' end,
        'highlights', '[]'::jsonb),
      'correlations', jsonb_build_object(
        'summary', case
          when finished_game_count < 3 then 'Need at least 3 games for meaningful correlation analysis.'
          else concat(jsonb_array_length(correlations_items)::text, ' stats analyzed across ', finished_game_count::text, ' games.') end,
    ...
    'insightsScreen', jsonb_build_object(
      ...
      'correlations', jsonb_build_object(
        'summary', case
          when finished_game_count < 3 then 'Need at least 3 games for meaningful correlation analysis.'
          else 'Win/loss correlations across objectives, assists, and failures.' end,
        'items', correlations_items,
        'selectedKey', null)
    ),
  ...
  select g.id,
    jsonb_build_object('groupId', g.id, 'name', g.name,
      'gamesPlayed', coalesce(gc.game_count, 0), 'lastGameId', gc.last_game_id,
  ...
  left join lateral (
    ...
    from public.games as games where games.group_id = g.id and games.status = 'finished'
  ) as gc on true
  left join lateral (
    select count(*)::int as member_count from public.group_members as gm where gm.group_id = g.id
  ) as mc on true
...
-- Backfill all profiles with new fields
do $$
declare p record;
begin
