...
-- Blend non-prestige score contribution into ELO with a smaller influence than wins.
-- Bonus score uses score - total_prestige, which captures contracts, assists,
-- failures, and head-to-head mission score bonuses without double-counting prestige.
...
declare
  ...
  bonus_min numeric;
  bonus_max numeric;
  bonus_normalized numeric;
  k          constant numeric := 32;
  s_elo      constant int     := 1000;
  conf_cap   constant int     := 12;
  elo_result_weight constant numeric := 0.8;
  elo_bonus_score_weight constant numeric := 0.2;
...
begin
  ...
  for game_rec in
    select
      ...
      ) filter (where gp.profile_id is not null) as bonus_scores
  ...
  loop
    ...
    into bonus_min, bonus_max
    from unnest(game_rec.bonus_scores) as value;
    ...
    for i in 1..n loop
      ...
      bonus_score := coalesce(game_rec.bonus_scores[i], 0);
      bonus_normalized := case
        when bonus_max is not null and bonus_min is not null and bonus_max > bonus_min
          then (bonus_score - bonus_min) / (bonus_max - bonus_min)
        else 0.5
      end;
      base_actual := case
      ...
      actual_s := greatest(
        ...
        least(
          1::numeric,
          base_actual * elo_result_weight + bonus_normalized * elo_bonus_score_weight
        )
      );
