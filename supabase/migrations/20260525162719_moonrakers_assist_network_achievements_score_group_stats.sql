...
-- Full rewrite of admin_refresh_analytics adding:
-- #1  assistNetwork nodes + edges (insightsScreen)
-- #2  memberStats + topWinner in group_stats_rollups
-- #3  achievements section (new top-level key)
-- #4  score avgScore / scoreDescription / score correlation item
...
declare
  ...
  signal_avg_assists numeric := 0; signal_avg_failures numeric := 0;
  ...
  -- insights correlations (from round 7)
  ...
  -- #1 assist network; #3 achievements
  achievements_payload jsonb := '[]'::jsonb;
  assist_network_nodes jsonb := '[]'::jsonb;
  assist_network_edges jsonb := '[]'::jsonb;
...
begin
  ...
  -- Consolidated participant query — #4 adds avg_score_win/lose
  select
  ...
  -- #4: score added as 4th correlation item alongside objectives/assists/failures
  if finished_game_count>=3 then
  ...
  -- #4: avgScore and scoreDescription added to player_detail.stats
  player_detail:=jsonb_build_object('playerId',target_profile_id,'label',coalesce(target_display_name,target_player_name,'Current player'),'summary',case when finished_game_count>0 then concat('Server-authored player detail across ',finished_game_count::text,' finished game',case when finished_game_count=1 then '' else 's' end,'.') else 'No finished games are available for this player yet.' end,'stats',jsonb_build_object('games',finished_game_count,'wins',signal_win_count,'winRate',concat(round(signal_win_rate*100),'%'),'playerRows',player_row_count,'avgPrestige',round(ps_avg_direct+ps_avg_assist_recv+ps_avg_objective,1),'contractConversion',concat(round(signal_contract_conversion*100),'%'),'totalScore',lifetime_score,'totalPrestige',lifetime_prestige,'avgScore',avg_score_per_game,'scoreDescription','Score tracks cumulative round performance across the full game. Unlike prestige (your final tally), score reflects how many turns you actively scored — a high score with low prestige means consistent but small contributions.'));

  -- #3: Achievements — computed from existing data, no extra tables needed
  if finished_game_count > 0 then
    with base_games as (
      select gp.game_id, gp.is_winner, gp.failures, gp.assists, gp.total_prestige, gp.score,
        coalesce(g.finished_at, g.created_at) as finished_at,
        (select max(gp2.total_prestige) from public.game_participants as gp2
         where gp2.game_id=gp.game_id and not gp2.is_winner) as second_prestige
      from public.game_participants as gp
      join public.games as g on g.id=gp.game_id and g.status='finished'
      where gp.profile_id=target_profile_id
    ),
    streak_tagged as (
      select is_winner, finished_at, game_id,
        row_number() over (order by finished_at asc, game_id asc) -
        row_number() over (partition by is_winner order by finished_at asc, game_id asc) as grp
      from base_games
    ),
    win_streaks as (
      select count(*)::int as streak_len, max(finished_at) as streak_end
      from streak_tagged where is_winner group by grp
    ),
    comeback_games as (
      select bg.game_id, bg.finished_at
      from base_games as bg
      where bg.is_winner
        and (
          select coalesce(sum(gr.prestige) filter (where gr.round_index < mri.max_ri/2.0), 0)
          from public.game_participants as my_gp
          join public.game_rounds as gr on gr.participant_id = my_gp.id
          join lateral (select max(round_index)::float as max_ri from public.game_rounds where participant_id=my_gp.id) as mri on true
          where my_gp.game_id = bg.game_id and my_gp.profile_id = target_profile_id
        ) < coalesce((
          select max(opp_half)
          from (
            select coalesce(sum(gr2.prestige) filter (where gr2.round_index < mri2.max_ri/2.0), 0) as opp_half
            from public.game_participants as opp_gp
            join public.game_rounds as gr2 on gr2.participant_id = opp_gp.id
            join lateral (select max(round_index)::float as max_ri from public.game_rounds where participant_id=opp_gp.id) as mri2 on true
            where opp_gp.game_id=bg.game_id and opp_gp.profile_id is not null and opp_gp.profile_id!=target_profile_id
            group by opp_gp.id
          ) as opp_halves
        ), 0)
    ),
    ach_raw as (
      select 'first_win'::text as key, 'First Victory'::text as label,
        'Won your first tracked game.'::text as description,
        min(finished_at) as earned_at, null::numeric as value
      from base_games where is_winner having count(*) filter (where is_winner) > 0
      union all
      select 'perfect_win','Flawless Win','Won a game without a single failure.',
        min(finished_at) filter (where is_winner and failures=0), null
      from base_games having count(*) filter (where is_winner and failures=0) > 0
      union all
      select 'dominant_win','Dominant Victory','Won a game by 5 or more prestige.',
        min(finished_at) filter (where is_winner and (total_prestige-coalesce(second_prestige,0))>=5), null
      from base_games having count(*) filter (where is_winner and (total_prestige-coalesce(second_prestige,0))>=5) > 0
      union all
      select 'nail_biter','Nail-Biter','Won a game by exactly 1 prestige.',
        min(finished_at) filter (where is_winner and (total_prestige-coalesce(second_prestige,0))=1), null
      from base_games having count(*) filter (where is_winner and (total_prestige-coalesce(second_prestige,0))=1) > 0
      union all
      select 'high_scorer','High Scorer','Scored 50 or more points in a single game.',
        min(finished_at) filter (where score>=50), max(score) filter (where score>=50)
      from base_games having count(*) filter (where score>=50) > 0
      union all
      select 'assist_master','Assist Master','Recorded 3 or more assists in a single game.',
        min(finished_at) filter (where assists>=3), max(assists) filter (where assists>=3)::numeric
      from base_games having count(*) filter (where assists>=3) > 0
      union all
      select 'win_streak_3','Hot Streak','Won 3 games in a row.',
        min(streak_end) filter (where streak_len>=3), max(streak_len) filter (where streak_len>=3)::numeric
      from win_streaks having count(*) filter (where streak_len>=3) > 0
      union all
      select 'win_streak_5','On Fire','Won 5 games in a row.',
        min(streak_end) filter (where streak_len>=5), max(streak_len) filter (where streak_len>=5)::numeric
      from win_streaks having count(*) filter (where streak_len>=5) > 0
      union all
      select 'comeback_win','Comeback Kid','Won a game after trailing at halftime.',
        min(finished_at), null
      from comeback_games having count(*) > 0
    )
    select coalesce(jsonb_agg(
      jsonb_build_object('key',key,'label',label,'description',description,'earnedAt',earned_at,'value',value)
      order by earned_at asc
    ), '[]'::jsonb)
    into achievements_payload
    from ach_raw;
  end if;

  -- Insights correlations (from round 7 — unchanged)
  if finished_game_count >= 2 then
    ...
    from (
      ...
      from (select distinct on (gp.profile_id) gp.profile_id, coalesce(nullif(gp.display_name_snapshot,''),nullif(gp.player_name_snapshot,''),'Unknown') as label from public.game_participants as gp join public.game_participants as my_gp on my_gp.game_id=gp.game_id and my_gp.profile_id=target_profile_id join public.games as g on g.id=gp.game_id and g.status='finished' where gp.profile_id is not null and gp.profile_id<>target_profile_id order by gp.profile_id, label) as np
      cross join (select g.id as game_id, exists(select 1 from public.game_participants as wgp where wgp.game_id=g.id and wgp.profile_id=target_profile_id and wgp.is_winner) as target_won from public.games as g join public.game_participants as my_gp on my_gp.game_id=g.id and my_gp.profile_id=target_profile_id where g.status='finished') as tg
      left join public.game_participants as paired on paired.game_id=tg.game_id and paired.profile_id=np.profile_id
      group by np.profile_id, np.label having count(paired.profile_id)>=2
      order by abs(coalesce(corr(case when paired.profile_id is null then 0::double precision else 1::double precision end, case when tg.target_won then 1::double precision else 0::double precision end)::numeric,0)) desc limit 6
    ) as p;
    ...
    -- #1: Assist network nodes + edges (previously always empty stubs)
    with tg as (
    ...
    net_players as (
      select distinct on (gp.profile_id)
        gp.profile_id,
        coalesce(nullif(p.display_name,''), p.player_name, 'Unknown') as display_name,
        p.favorite_color,
    ...
    assist_flows as (
      select
        ...
        count(*)::int as times_assisted,
        sum((edge.v)::numeric) as total_prestige
      ...
      join lateral jsonb_each_text(gr.assist_prestige_recipients) as edge(k,v) on true
      join public.game_participants as rec
        on rec.game_id = gr.game_id
        and rec.profile_id::text = btrim(edge.k)
      where btrim(edge.k) != '' and (edge.v)::numeric > 0
        and src.profile_id is not null and rec.profile_id is not null
    ...
    select
      coalesce((
        select jsonb_agg(jsonb_build_object(
          'id', np.profile_id,
          'label', np.display_name,
          'color', np.favorite_color,
          'assignedCardArtIndex', np.assigned_card_art_index,
          'assistsGiven',    coalesce(ag.assists_given, 0),
          'prestigeGiven',   coalesce(ag.prestige_given, 0),
          'assistsReceived', coalesce(ar.assists_received, 0),
          'prestigeReceived',coalesce(ar.prestige_received, 0)
        ))
        from net_players as np
        left join (select from_id as pid, sum(times_assisted)::int as assists_given,   sum(total_prestige) as prestige_given   from assist_flows group by from_id) as ag on ag.pid = np.profile_id
        left join (select to_id   as pid, sum(times_assisted)::int as assists_received, sum(total_prestige) as prestige_received from assist_flows group by to_id)   as ar on ar.pid = np.profile_id
      ), '[]'::jsonb),
      coalesce((
        select jsonb_agg(jsonb_build_object(
          'fromId', af.from_id, 'toId', af.to_id,
          'timesAssisted', af.times_assisted, 'totalPrestige', af.total_prestige
        ))
        from assist_flows as af
      ), '[]'::jsonb)
  ...
  analytics_payload:=jsonb_build_object(
    ...
    'insightsScreen',jsonb_build_object('generatedAt',generated_at,
      ...
      -- #1: populated assist network replaces empty stub
      'assistNetwork',jsonb_build_object('nodes',assist_network_nodes,'edges',assist_network_edges),
      'correlations',jsonb_build_object(
        ...
        'personal',personal_payload,'pairing',pairing_payload,'macro',macro_payload,'synergyPairs',synergy_payload,
        'players','[]'::jsonb,'items',correlations_items,'selectedKey',null,'winLoseSplit',correlations_items)));
  ...
  -- #2: Extended group rollup with memberStats and topWinner
  ...
  select
    ...
    jsonb_build_object(
      ...
      'memberStats',  coalesce(ms.stats, '[]'::jsonb),
      'topWinner',    ms.top_winner_name
    ),
    now()
  from public.groups as g
  left join lateral (
    select count(*)::int as game_count,
      (array_agg(gg.id order by gg.created_at desc, gg.id desc))[1] as last_game_id
    from public.games as gg where gg.group_id=g.id and gg.status='finished'
  ) as gc on true
  left join lateral (
  ...
  left join lateral (
    select
      coalesce(jsonb_agg(
        jsonb_build_object(
          'profileId',  p.id,
          'displayName',coalesce(nullif(p.display_name,''), p.player_name, 'Player'),
          'games',      ms_inner.games,
          'wins',       ms_inner.wins,
          'winRate',    case when ms_inner.games>0 then round(ms_inner.wins::numeric/ms_inner.games,3) else 0 end,
          'avgPrestige',ms_inner.avg_prestige,
          'avgScore',   ms_inner.avg_score
        ) order by ms_inner.wins desc, ms_inner.avg_prestige desc
      ), '[]'::jsonb) as stats,
      (select coalesce(nullif(p2.display_name,''), p2.player_name)
       from public.profiles as p2
       where p2.id = (
         select gp2.profile_id
         from public.game_participants as gp2
         join public.games as gg2 on gg2.id=gp2.game_id and gg2.group_id=g.id and gg2.status='finished'
         where gp2.profile_id is not null
         group by gp2.profile_id
         order by count(*) filter (where gp2.is_winner) desc, count(*) desc
         limit 1
       )) as top_winner_name
    from (
      ...
      from public.game_participants as gp
      join public.games as gg on gg.id=gp.game_id and gg.group_id=g.id and gg.status='finished'
      where gp.profile_id is not null
      group by gp.profile_id
    ) as ms_inner
    join public.profiles as p on p.id = ms_inner.profile_id
  ) as ms on true
...
-- Backfill all rollups with every new section
do $$
declare p record;
begin
...
-- #2: get_group_screen — reads the enriched group rollup, auth-gated to members/participants
create or replace function public.get_group_screen(target_group_id uuid)
returns jsonb
language plpgsql
stable
security definer
...
begin
  ...
  if not exists (
    ...
    where gm.group_id = target_group_id and gm.profile_id = viewer_id
  ) and not exists (
    select 1 from public.game_participants as gp
    join public.games as g on g.id = gp.game_id
      and g.group_id = target_group_id and g.status = 'finished'
    where gp.profile_id = viewer_id
  ) then
    raise exception 'access denied: not a member of or participant in this group';
  end if;
  ...
  if result is null then
    return jsonb_build_object(
      ...
      'emptyState', jsonb_build_object(
        'title',       'No data yet',
        'description', 'Play a finished game in this group to unlock group stats.'
      )
    );
  end if;
