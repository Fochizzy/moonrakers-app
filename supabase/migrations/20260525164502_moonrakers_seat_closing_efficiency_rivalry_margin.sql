...
-- Adds to admin_refresh_analytics:
-- #1  seat-bias top signals + group-level seatBias table
-- #2  closing-strength fields in paceProfile (avgOpeningTurns / avgClosingTurns)
-- #3  prestige-per-turn efficiency in player_detail.stats
-- #4  avgPrestigeMargin / avgWinMargin / lastResult / lastPlayedAt in rivalries
...
declare
  ...
  -- #2: closing-strength fields
  pace_avg_opening_turns      numeric := 0;
  pace_avg_closing_turns      numeric := 0;
  pace_avg_closing_turns_win  numeric := 0;
  pace_avg_closing_turns_loss numeric := 0;
  -- #3: prestige-per-turn
  avg_prestige_per_turn      numeric := 0;
  avg_prestige_per_turn_win  numeric := 0;
  avg_prestige_per_turn_loss numeric := 0;
  ...
  -- #1: seat-bias signal vars
  seat_best_order   integer := -1;
  seat_best_apps    integer := 0;
  seat_best_win_pct numeric := 0;
  seat_worst_order  integer := -1;
  seat_worst_apps   integer := 0;
...
begin
  ...
  -- #2 + #3: Closing strength and prestige-per-turn from ranked per-player turns
  -- player_turn = position within this player's own turn sequence (not global round_index)
  select
    coalesce(round(avg(opening_prestige), 2), 0)::numeric,
    coalesce(round(avg(closing_prestige), 2), 0)::numeric,
  ...
  into
    pace_avg_opening_turns, pace_avg_closing_turns, pace_avg_closing_turns_win, pace_avg_closing_turns_loss,
  ...
  from (
    ...
    join public.games as g on g.id=gp.game_id and g.status='finished'
    join (
      select participant_id, prestige,
        row_number() over (partition by participant_id order by round_index asc) as player_turn,
        count(*)     over (partition by participant_id)                           as total_turns
      from public.game_rounds
    ) as gr_r on gr_r.participant_id = gp.id
    where gp.profile_id = target_profile_id
    group by gp.id, gp.is_winner, gp.total_prestige
  ) as per_game;
  ...
  if finished_game_count>=3 then
    ...
    -- #1: Seat-bias signals — flag dominant (≥60% from a seat, 3+ games) or winless seats
    select start_order, appearances, round(wins::numeric/appearances, 3)
    into seat_best_order, seat_best_apps, seat_best_win_pct
    from (
      select gp.start_order, count(*)::int as appearances, count(*) filter (where gp.is_winner)::int as wins
      from public.game_participants as gp
      join public.games as g on g.id=gp.game_id and g.status='finished'
      where gp.profile_id=target_profile_id
      group by gp.start_order having count(*) >= 3
    ) as seat_data
    order by wins::numeric/appearances desc
    limit 1;

    if seat_best_order >= 0 and seat_best_win_pct >= 0.60 then
      top_signals := top_signals || jsonb_build_array(jsonb_build_object(
        'key','strong-seat','label',concat('Dominant from seat ',seat_best_order::text),
        'value',concat(round(seat_best_win_pct*100)::int::text,'% win rate (', seat_best_apps::text,' games)'),'tone','blue'));
    end if;

    select start_order, appearances
    into seat_worst_order, seat_worst_apps
    from (
      select gp.start_order, count(*)::int as appearances, count(*) filter (where gp.is_winner)::int as wins
      from public.game_participants as gp
      join public.games as g on g.id=gp.game_id and g.status='finished'
      where gp.profile_id=target_profile_id
      group by gp.start_order having count(*) >= 3 and count(*) filter (where gp.is_winner) = 0
      limit 1
    ) as seat_data;

    if seat_worst_order >= 0 then
      top_signals := top_signals || jsonb_build_array(jsonb_build_object(
        'key','weak-seat','label',concat('0 wins from seat ',seat_worst_order::text),
        'value',concat(seat_worst_apps::text,' games, 0% win rate'),'tone','danger'));
    end if;
  ...
  -- #4: Head-to-head with prestige margin + last result
  select coalesce(jsonb_agg(jsonb_build_object(
    ...
    'avgPrestigeMargin', avg_margin,
    'avgWinMargin',      avg_win_margin,
    'lastResult',        last_result,
    'lastPlayedAt',      last_played_at
  ) order by games_together desc, opponent_name asc), '[]'::jsonb)
  into head_to_head
  from (
    select
      ...
      round(avg(gpb.total_prestige - gpa.total_prestige), 1)                               as avg_margin,
      round(avg(gpb.total_prestige - gpa.total_prestige)
            filter (where g.winner_profile_id=target_profile_id), 1)                        as avg_win_margin,
      (array_agg(
         case when g.winner_profile_id=target_profile_id then 'W'
              when g.winner_profile_id=other_p.id         then 'L'
              else 'D' end
         order by coalesce(g.finished_at,g.created_at) desc
       ))[1]                                                                                as last_result,
      max(coalesce(g.finished_at,g.created_at))                                            as last_played_at
  ...
  -- #3: prestigePerTurn + closing strength added to stats; #2 fields flow into paceProfile below
  ...
  -- Achievements (#3 from round 9 migration — unchanged)
  if finished_game_count > 0 then
    with base_games as (
      select gp.game_id, gp.is_winner, gp.failures, gp.assists, gp.total_prestige, gp.score,
        ...
        (select max(gp2.total_prestige) from public.game_participants as gp2 where gp2.game_id=gp.game_id and not gp2.is_winner) as second_prestige
    ...
    comeback_games as (
      select bg.game_id, bg.finished_at from base_games as bg where bg.is_winner
      and (select coalesce(sum(gr.prestige) filter (where gr.round_index<mri.max_ri/2.0),0) from public.game_participants as my_gp join public.game_rounds as gr on gr.participant_id=my_gp.id join lateral (select max(round_index)::float as max_ri from public.game_rounds where participant_id=my_gp.id) as mri on true where my_gp.game_id=bg.game_id and my_gp.profile_id=target_profile_id)
  ...
  -- Insights correlations (from round 7/9 — unchanged)
  if finished_game_count >= 2 then
    ...
    from (select np.label, coalesce(corr(case when paired.profile_id is null then 0::double precision else 1::double precision end, case when tg.target_won then 1::double precision else 0::double precision end)::numeric,0) as corr_value, count(paired.profile_id)::int as games_together
      ...
      order by abs(coalesce(corr(case when paired.profile_id is null then 0::double precision else 1::double precision end, case when tg.target_won then 1::double precision else 0::double precision end)::numeric,0)) desc limit 6) as p;
  ...
  analytics_payload:=jsonb_build_object(
    ...
    'statsScreen',jsonb_build_object('generatedAt',generated_at,
      ...
      -- #2: paceProfile now includes closing / opening turn strength
  ...
  -- #1: Group rollup now includes seatBias (win rate by seat position across group games)
  ...
  select
    ...
    jsonb_build_object(
      ...
      'seatBias',    coalesce(sb.stats, '[]'::jsonb)
    ),
    now()
  ...
  left join lateral (
    select coalesce(jsonb_agg(
      jsonb_build_object('seat',sd.start_order,'appearances',sd.appearances,'wins',sd.wins,'winRate',case when sd.appearances>0 then round(sd.wins::numeric/sd.appearances,3) else 0 end)
      order by sd.start_order
    ), '[]'::jsonb) as stats
    from (
      select gp.start_order, count(*)::int as appearances, count(*) filter (where gp.is_winner)::int as wins
      from public.game_participants as gp
