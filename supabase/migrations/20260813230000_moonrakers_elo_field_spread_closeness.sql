...
-- Measure how close a game was from the whole field, not just the top two.
--
-- Closeness previously came from the winner's prestige margin over the
-- runner-up. That had three problems, all visible in the existing history:
--
--   * It saturated. Eight of 23 games pinned the multiplier at its 1.25x
--     ceiling while their underlying margins ranged from 0.5 to 2.0 of the
--     table average, and no game ever reached the 0.75x floor. The median game
--     sat at 1.10x, so the scale was skewed high rather than centred.
--   * It ignored everyone below second. In a four-player game the third and
--     fourth finishers had no influence on a multiplier that scaled their own
--     rating change.
--   * A comeback could read as a rout: one game rated fully decisive had the
--     winner leading only a third of the turns.
--
-- Closeness is now the mean absolute deviation of final prestige relative to
-- the table average, which reads every finisher. On the same history its median
-- is 0.271 against a 0.5 reference and only three games saturate, so the
-- multiplier uses its range and centres near 1.0x.
--
...
-- rating leak the finishing-position result base exists to prevent - which is
-- itself the argument for a whole-field statistic over a top-two one.
--
-- Trajectory-based closeness (how late the lead last changed) is the truest
-- signal but is deliberately not used here: the round rows under-account
-- prestige for 20 of 74 participants, so it would scale ratings partly on
-- reconstruction error. Final prestige totals are authoritative.
...
begin
  ...
  loop
    ...
    -- How close the game was: how far the whole field spread out, as the mean
    -- absolute deviation of final prestige relative to the table average.
