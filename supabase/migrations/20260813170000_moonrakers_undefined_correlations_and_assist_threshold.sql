-- Stops correlation cards claiming a real 0.00 when the statistic is undefined, and
-- makes the assist-behind-leader threshold reachable.
--
-- Every card wrapped its corr() in coalesce(..., 0), so three different failure
-- modes all rendered as an identical, confident "0.00":
--   * assistsOverFiveBehindLeader -- the gap-to-leader never exceeded 5 in any
--     recorded assist, so the flag column was constant and corr() was undefined;
--   * the pairing cards for teammates who play in every single game -- a constant
--     predictor, so corr() is undefined;
--   * a genuine zero, which looks exactly the same.
-- Undefined now flows through as NULL and the card reports "No data".
--
-- The threshold moves from >5 to >2. Observed gap-to-leader across all finished
-- games tops out at exactly 5 (46/29/20/8/2/1 events at gaps 0..5), so ">5 behind"
-- could never fire. Prestige totals run 8-12 a game, so being 3 back is already
-- meaningfully behind; >2 catches 11 events across 9 game-player rows. That sample
-- is thin, and if it ever becomes unreachable again the NULL path above reports it
-- honestly instead of silently returning to 0.00.
--
-- The internal key stays assistsOverFiveBehindLeader. Renaming it is a server ->
-- client contract change (glossary, definition targets, the
-- assistsOverFiveBehindLeaderLabel payload field, the profile component and six
-- test files) and belongs in its own change, not bundled with a semantics fix.
--
...
-- Builds one correlation card. Lives in public, not private, for the same reason
-- seat_advantage_spread does: get_insights_screen is not security definer, so it
-- executes as the caller and cannot reach the private schema. Reads no tables.
--
-- A NULL value means the statistic could not be computed -- no variance in the
-- predictor, or too few samples -- which is reported as such rather than as 0.
create or replace function public.correlation_card(
  card_key text,
  metric_key text,
  label text,
  value numeric
...
as $$
  select jsonb_build_object(
    ...
    'label', label,
    'value', case when value is null then null else round(value, 2) end,
    'strength', case
      when value is null then 'No data'
      when abs(value) >= 0.5 then 'Strong'
      when abs(value) >= 0.25 then 'Moderate'
      else 'Light'
    end
  );
$$;

revoke all on function public.correlation_card(text, text, text, numeric) from public;
revoke all on function public.correlation_card(text, text, text, numeric) from anon;
grant execute on function public.correlation_card(text, text, text, numeric) to authenticated;
...
as $$
  ...
  select case
    when bounds.seat_count < 2 then null::numeric
    when bounds.best_seat is null or bounds.worst_seat is null then null::numeric
...
begin
  ...
  if finished_game_count >= 2 then
    ...
    ranked_pairings as (
      ...
      order by abs(pairing_metrics.corr_value) desc nulls last, pairing_metrics.games_together desc, pairing_metrics.label
      limit 6
    )
    select coalesce(
      jsonb_agg(
        public.correlation_card(
          null,
          null,
          format('With %s vs win rate', ranked_pairings.label),
          ranked_pairings.corr_value
        )
        order by abs(ranked_pairings.corr_value) desc nulls last, ranked_pairings.games_together desc, ranked_pairings.label
      ),
      '[]'::jsonb
    )
    ...
    assist_events as (
      select
        ...
        case
          when (leader_state.leader_prestige - helper_state.pbr) > 2 then 1
          else 0
    ...
    personal_payload := jsonb_build_array(
      public.correlation_card(
        'baseRate',
        'baseRate',
        'Stay at Base Rate vs Victory',
        personal_base_rate_win
      ),
      public.correlation_card(
        'baseRateObjective',
        'baseRate',
        'Stay at Base Rate vs Objective Points',
        personal_base_rate_objectives
      ),
      public.correlation_card(
        'turnOrderWinCorrelation',
        'turnOrderWinCorrelation',
        'Seat Advantage Spread',
        personal_seat_to_win
      ),
      public.correlation_card(
        'lateLeadConversion',
    ...
    macro_payload := jsonb_build_array(
      ...
      public.correlation_card(
        ...
        'assistsOverFiveBehindLeader',
        'Assists 3+ Behind Leader vs Victory',
        macro_assists_over5_behind
      ),
      public.correlation_card(
