...
export type EnrichedPlayerStats = DerivedPlayerStats & {
  ...
  efficiency: number; // All Eff
  assistedEfficiency: number; // Assist Eff
...
export function enrichPlayerStats(base: DerivedPlayerStats): EnrichedPlayerStats {
  ...
  const efficiency = safeDiv(totalPrestige, contracts + assistsGiven);
  const assistedEfficiency = safeDiv(base.assistPrestigeReceived, assistsGiven);
  const directEfficiency = safeDiv(base.directPrestige, contracts);
