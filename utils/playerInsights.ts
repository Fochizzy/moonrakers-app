...
export function buildPlayerInsights(
  ...
  const directEfficiency = totalContracts > 0 ? totalDirectPrestige / totalContracts : 0;
  const assistedEfficiency = totalAssists > 0 ? totalAssistPrestigeReceived / totalAssists : 0;
  const efficiency =
    totalContracts + totalAssists > 0
  ...
  insights.push(
    `Efficiency profile: All Eff ${efficiency.toFixed(2)}, Direct Eff ${directEfficiency.toFixed(2)}, Assist Eff ${assistedEfficiency.toFixed(2)}.`
  );
  ...
  if (assistedEfficiency > efficiency + 0.15) {
    insights.push('Performance improves noticeably when supported by teammates.');
  } else if (efficiency > assistedEfficiency + 0.15) {
    insights.push('Converts direct contracts into prestige more efficiently than overall team involvement.');
  } else if (totalContracts + totalAssists > 0) {
    insights.push('Solo and team-supported efficiency are fairly balanced.');
  }
