...
function CompactPlayerStrip({
  ...
  const ranking = [...players].sort((a, b) => {
    const totalA = getTotalPrestigeFromTotals(totals[a.id] as PlayerTotals) || 0;
    const totalB = getTotalPrestigeFromTotals(totals[b.id] as PlayerTotals) || 0;
    if (totalB !== totalA) return totalB - totalA;

    const scoreA = toNumber(totals[a.id]?.score);
    const scoreB = toNumber(totals[b.id]?.score);
...
export default function Game() {
  ...
  function toggleAssist(playerId: string, next: 0 | 1) {
    ...
    if (next === 1) {
      assistPrestigeRecipients[playerId] = Math.max(
        0,
        toNumber(assistPrestigeRecipients[playerId])
      );
  ...
  function buildCandidateTotals(candidateRounds: any[]) {
  ...
  function saveRoundAndAdvance() {
    try {
      ...
      const nextTotals = buildCandidateTotals(candidate.nextRounds);
  ...
  return (
    <View style={[styles.screen, { backgroundColor: appBackground }]}>
      ...
      <ScrollView ref={scrollViewRef} showsVerticalScrollIndicator={false} contentContainerStyle={styles.container}>
        ...
        <CompactPlayerStrip players={players} activePlayerId={currentPlayer.id} totals={totals} />
